...
class MALProfile:
    ...
    def scraper(self):
        ...
        **Raises:**
        ...
        _website_response = requests.get(self.url)
        if _website_response:
            self.__class_dict.setdefault("Status Code", _website_response.status_code)
            soup = BeautifulSoup(_website_response.text, 'html.parser')
            ...
            raise Exception(f"Non-success status code: {_website_response.status_code}")
